...
class FilterPlugin(BaseAdminPlugin):
    ...
    def get_list_queryset(self, queryset):
        ...
        self.admin_view.clean_query_url = self.admin_view.get_query_string(remove=[k for k in list(self.request.GET.keys()) if
        ...
        obj = [f for f in self.filter_specs if f.is_used]
        if six.PY3:
            obj = list(obj)
    ...
    def get_media(self, media):
        arr = [s for s in self.filter_specs if isinstance(s, DateFieldListFilter)]
        if six.PY3:
            arr = list(arr)
        if bool(arr):
            media = media + self.vendor('datepicker.css', 'datepicker.js',
                                        'xadmin.widget.datetime.js')
        arr = [s for s in self.filter_specs if isinstance(s, RelatedFieldSearchFilter)]
        if six.PY3:
            arr = list(arr)
        if bool(arr):
