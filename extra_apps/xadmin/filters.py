...
class BaseFilter(object):
    ...
    def form_params(self):
        arr = [FILTER_PREFIX + k for k in list(self.used_params.keys())]
        if six.PY3:
            arr = list(arr)
...
class FieldFilter(BaseFilter):
    ...
    def __init__(self, field, request, params, model, admin_view, field_path):
        ...
        arr = [setattr(self, 'lookup_' + kv[0], kv[1]) for kv in list(self.context_params.items())]
        if six.PY3:
            list(arr)
    ...
    def get_context(self):
        ...
        obj = [FILTER_PREFIX + k for k in list(self.used_params.keys())]
        if six.PY3:
            obj = list(obj)
