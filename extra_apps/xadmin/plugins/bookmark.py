...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        current_qs = '&'.join([
                '%s=%s' % (k, v)
                for k, v in sorted([i for i in list(self.request.GET.items()) if bool(i[1] and (
        ...
        for bk in self.list_bookmarks:
            ...
            params = dict([
                    (FILTER_PREFIX + k, v)
                    for (k, v) in list(bk['query'].items())
                    ])
            if 'order' in bk:
            ...
            bk_qs = '&'.join([
                    '%s=%s' % (k, v)
                    for k, v in sorted(filter(check_item, list(params.items())))
                    ])
...
class BookmarkWidget(PartialBaseWidget):
    ...
    def context(self, context):
        ...
        context['results'] = [
                [o for i, o in enumerate([c for c in r.cells if c.field_name in base_fields])]
